...
class Trainer:
    def __init__(self, model, train_loader, val_loader, optimizer, scheduler, device, config, experiment_path, class_weights=None, writer=None, initial_history=None):
        ...
        else: self.history = {"train_loss": [], "val_loss": [], "train_acc": [], "val_acc": [], "train_f1": [], "val_f1": [], "lr": []}
        self.use_amp = self.config['training'].get('use_mixed_precision', False)
        self.scaler = GradScaler(enabled=self.use_amp)
        if self.use_amp: print("Automatic Mixed Precision (AMP) is activated.")
        self.early_stopper = None
        es_config = self.config.get('training', {}).get('early_stopping', {})
        if es_config.get('enabled', False):
            self.early_stopper = EarlyStopping(patience=es_config.get('patience', 10), min_delta=es_config.get('min_delta', 0), monitor=es_config.get('monitor', 'val_loss'), mode=es_config.get('mode', 'min'))
    ...
    def _compute_loss(self, logits, targets):
        ...
        loss = self.model.softmax_groups.get_loss(preds=logits_flat, targets=targets_permuted, class_weights=self.class_weights, focal=self.config['loss'].get('use_focal', True), gamma=self.config['loss'].get('focal_loss_gamma', 2.0), include_silence=self.config['data']['include_silence'])
        return loss
    ...
    def _evaluate(self, data_loader):
        ...
        if not all_preds: return {'val_loss': avg_loss, 'val_acc': 0, 'val_f1': 0}
        all_preds, all_targets = torch.cat(all_preds, dim=0), torch.cat(all_targets, dim=0)
        include_silence, silence_class = self.config['data']['include_silence'], self.config['data'].get('silence_class', 20)
    ...
    def _save_checkpoint(self, epoch, best_val_f1, is_best=False, is_interval=False):
        ...
        state = {'epoch': epoch, 'model_state_dict': self.model.state_dict(), 'optimizer_state_dict': self.optimizer.state_dict(), 'scheduler_state_dict': self.scheduler.state_dict() if self.scheduler else None, 'best_val_f1': best_val_f1, 'history': self.history}
        ...
        for epoch in range(start_epoch, self.config['training']['epochs']):
            ...
            if self.scheduler:
                if isinstance(self.scheduler, ReduceLROnPlateau):
                    self.scheduler.step(val_metrics[self.config.get('training', {}).get('scheduler', {}).get('params', {}).get('monitor', 'val_loss')])
                else:
                    self.scheduler.step()
